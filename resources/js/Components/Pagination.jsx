...
const Pagination = ({ ...props }) => {
    const linkPagination = () => {
        const maxPaginate =
            props.data.last_page < 5 ? props.data.last_page - 1 : 4;
        let generateLink = [];
        let startOn;
        if (props.data.last_page < 5) {
            startOn = 1;
        }
        if (props.data.current_page <= 3) {
            startOn = 1;
        } else if (props.data.last_page - props.data.current_page < 2) {
            startOn = props.data.last_page - 4;
        } else {
            startOn = props.data.current_page - 2;
        }

        for (let index = startOn; index <= startOn + maxPaginate; index++) {
            generateLink.push(props.data.links[index]);
        }
        return generateLink;
    };
